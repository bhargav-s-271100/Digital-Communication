...
%Generation of random bits for input
N=20; 
s=randi(1,N); %Generating N bits
signal=[];  
carrier=[];
T=180;
...
title('Original Bit Sequence'); %plot original data
ylabel('Amplitude(V)');
xlabel('Time(s)');
...
% Power Spectrum Of Fast Frequency hopped signal
figure("Name", "Spectrum")
power_spectrum=abs(fft(xcorr(freq_hopped_sig)));
plot(power_spectrum/max(power_spectrum))
xlabel('Frequency')
ylabel('PSD')
title('Power Spectrum Of Fast Frequency hopped signal');
